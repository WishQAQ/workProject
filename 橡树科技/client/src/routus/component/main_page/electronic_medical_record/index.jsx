...
import {Left} from './componet/left'
...
//查询用户信息（单个）
  ...
  //循环返回参数
  backParam = (clinicRecList, type, date, key1, key2) => {
    let t;
    let obj = {};
    switch (type) {
      case '门诊':
    ...
    for (let i = 0; i < clinicRecList.length; i++) {
      if (moment(clinicRecList[i].timeFrom).format("YYYY-MM-DD") === date && t === clinicRecList[i].clinicType) {
  ...
  content = (active, key, other) => {
    ...
    let arr, date, type, data = {},dept;
    if (active !== '检查' || active !== '检验') {
      arr = key.split(',')
    }
    switch (active) {
      case '病案首页':
        ...
        data = this.backParam(clinicRecList, type, date, 'hospitalCode', 'patientId');
        break;
      case '体温单':
        if (arr.length === 3) {
          console.log(arr);
        ...
        for (let i = 0; i < clinicRecList.length; i++) {
          if (moment(clinicRecList[i].timeFrom).format("YYYY-MM-DD") === date && t === clinicRecList[i].clinicType) {
            ...
            obj['timeFrom'] = clinicRecList[i].timeFrom;
            obj['timeTo'] = clinicRecList[i].TimeTo;
            obj['hospitalName'] = clinicRecList[i].hospitalName;
            break;
          }
        }
        data = this.backParam(clinicRecList, type, date, 'hospitalCode', 'patientId');
        this.setState({
          temperatureParam: obj
        });
        break;
      case '处方记录':
        if (arr.length === 3) {
      ...
      case '门诊病历':
        ...
        data = this.backParam(clinicRecList, type, date, 'hospitalCode', 'outPatientNumber');
        break;
      case '处方':
        if (arr.length === 3) {
          date = arr[1].substring(2, arr[1].indexOf('\n'));
      ...
      case '检验':
        ...
        let param = {};
        if (other.length === 3) {
          date = other[1].substring(2, other[1].indexOf('\n'));
          type = other[1].substring(0, 2);
          dept=other[1].substring(other[1].indexOf('\n'),other[1].length);
        } else {
          let begin = other[3].indexOf('.');
          let y = other[3].substring(0, begin);
          let yearMotnDay = y + '.' + other[2];
          date = moment(yearMotnDay).format('YYYY-MM-DD');
          type = other[1].substring(0, other[1].indexOf('\n'));
        }
        data = this.backParam(clinicRecList, type, date, 'hospitalCode', 'patientId');
        if (type === '门诊') {
