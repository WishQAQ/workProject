...
export default function proxyMiddleware(options) {
  ...
  return function (req, resp, next) {
    ...
    const myReq = request(opts, function (myRes) {
      const statusCode = myRes.statusCode, headers = myRes.headers, location = headers.location;
      // Fix the location
...
function extend(obj, src) {
  for (let key in src) if (owns.call(src, key)) obj[key] = src[key];
  return obj;
}
